function [detections] = compare_detections_with_catalog(ac, fpss, catTimes, baseDir, dataFolder)
...
for k=1:length(fpss.thresh)
    ...
    [detections.fpss(k), fpss, ind_match_fpss, ind_new_fpss, ind_missed_fpss] = ...
        find_matching_detections_above_thresh(k, time_window, num_fp, ncatalog, fpss, catTimes);
    ...
    if (flag_output_fpss)
        ...
        C = [fpss.times(ind_match_fpss) fpss.sim(ind_match_fpss) fpss.catind(ind_match_fpss) fpss.cattime(ind_match_fpss)];
        fprintf(fid, '%7.5f %6.5f %d %7.5f\n', C');
        fprintf(fid, '\n');
...
for k=1:length(ac.thresh)
    ...
    [detections.ac(k), ac, ind_match_ac, ind_new_ac, ind_missed_ac] = ...
        find_matching_detections_above_thresh(k, time_window, num_ac, ncatalog, ac, catTimes);
    ...
    if (flag_output_autocorr)
        ...
        C = [ac.times(ind_match_ac) ac.sim(ind_match_ac) ac.catind(ind_match_ac) ac.cattime(ind_match_ac)];
        fprintf(fid, '%7.5f %10.9f %d %7.5f\n', C');
        fprintf(fid, '\n');
...
function [detinfo_k, newAlg, ind_match, ind_new, ind_missed] = ...
        find_matching_detections_above_thresh(k, time_window, num_newAlg, ncatalog, newAlg, catTimes)
