...
path(path,'./MatSAC');
[t,x,SAChdr] = fget_sac('../data/ncsn/1week.2011.008.00.00.00.0000.deci5.NC.CCOB..EHN.D.SAC.bp4to10'); % bandpass 4-10 Hz
% [t,x,SAChdr] = fget_sac('../data/ncsn/1week.2011.008.00.00.00.0000.deci5.NC.CCOB..EHN.D.SAC'); % unfiltered data
samplingRate = 20; % samples per second


%-------------------------------------------------------
% Load list of detections from fingerprint/similarity search
folder = '../data/haar_coefficients/NCSN_CCOB_EHN_1wk/';
filename = 'fpss_wLen10_wLag0.1_fpLen100_fpLag10_tvalue800_nfuncs5_ntbls100_nvotes4_timewin21_thresh0.08.mat';
filepath = strcat(folder, filename);
load(filepath);
dt = 1.0;
thresh = 0.19; % FPSS threshold
flag_plot_fp = 1;
flag_plot_ac = 0;
...
% detection_codes(ind_catalog) = 1;
% 
% % Detections missed by FPSS, found in autocorrelation
...
detection_samples = int64(detection_times * samplingRate);
...
if (flag_plot_fp)
    % Plot catalog events (21) - found with FPSS
...
%         '395178.84';'583296.05']); % regular catalog
    ...
    % Plot false detections (13) - found with FPSS
    ...
    % Plot new events (21) - found with FPSS - not found in autocorrelation
    ...
    % Plot new events (22/43: 1-22) - found with FPSS - also found in autocorrelation
    FigHandle = figure('Position',[1500 150 300 1000]);
    set(gca,'YDir','normal');
    set(gca,'FontSize',font_size,'FontWeight','bold');
    hold on
...
%     title('FPSS new events, also in autocorr, CCOB.EHN 1 week');

    % Plot new events (21/43: 23-43) - found with FPSS - also found in autocorrelation
    FigHandle = figure('Position',[1500 150 300 1000]);
    set(gca,'YDir','normal');
    set(gca,'FontSize',font_size,'FontWeight','bold');
    hold on
...
% Plot missed events (22) - found with autocorrelation - not found with FPSS
if (flag_plot_ac)
...
%     title('FPSS missed new events, found in autocorr, CCOB.EHN 1 week');
