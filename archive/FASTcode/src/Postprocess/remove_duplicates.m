function [topdata] = remove_duplicates(cc_i, cc_j, cc_values, skip_samples)
...
topdata.cc = zeros(preallocsize, 1, 'single');
top_k = 1;
for k = 1:nx_orig
    k_sort = ix(k);
    ...
    if (top_k > preallocsize)
        ...
        found_similar_ii = length(find( abs(search_cc_i-int32(cc_i(k_sort))) < skip_samples ));
        found_similar_jj = length(find( abs(search_cc_j-int32(cc_j(k_sort))) < skip_samples ));

        if (found_similar_ii && found_similar_jj)
            continue;
        else
            topdata.cc_i(top_k) = int32(cc_i(k_sort));
            topdata.cc_j(top_k) = int32(cc_j(k_sort));
