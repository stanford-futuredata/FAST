% Driver for FAST
% See get_FAST_input_parameters.m for input parameters
close all
clear

dataOutputDir = '/data/beroza/ceyoon/FASTcode/data/OutputFAST/';
% dataOutputDir = '../data/OutputFAST/';

% Set up paths to functions
addpath(genpath('./'));

[mem, u] = logmemory();
disp(['MEMORY, start: ' num2str(mem) ' ' u]);

% Get Fingerprint And Similarity Thresholding input parameters
inp = get_FAST_input_parameters();
...
baseDir = strcat(dataOutputDir, inp.fileNameStr{ifile},'/');
...
if (2 ~= exist(fpFileName, 'file')) % <<<<--------------- binary fingerprint file does not exist
    ...
    if (2 ~= exist(firstFpFileName, 'file')) % <<<<--------------- binary fingerprint partition file does not exist
       ...
       if (2 ~= exist(firstHaarFileName, 'file')) % <<<<--------------- normalized Haar image partition file does not exist
           ...
           [t, x, samplingRate] = get_channel_data(inp.timeSeriesFile{ifile}, inp.fileNameStr{ifile});
           s = size(x);
           nsamples = s(1);
           nch = s(2);
           ...
           % Loop over channels
           for k=1:nch
               % Set up spectrogram directory and filename
               specDir = strcat(baseDir, 'spectrogram/');
               specName = strcat(inp.specFolderStr{ifile}{k}, wStr);
               
               % Set up spectrogram settings
               specSettings(k) = setup_spectrogram_settings(nsamples, samplingRate, ...
                      inp.numPartitions, specDir, specName, inp.windowDuration, inp.windowLag);

               % If spectrogram does not exist, create and save it
               specFile = strcat(specDir, 'spec_', specName, '_part1.mat');
               if (2 ~= exist(specFile, 'file')) % <<<<--------------- spectrogram file does not exist
       ...
       for p=1:inp.numPartitions
           ...
           [binaryFp_p, partRuntime] = get_fingerprints(normHaarImages, inp.t_value); clear normHaarImages;
...
if (flag_plot_detections)
    ...
    out_dir = baseDir;
    outfile = [out_dir inp.fileNameStr{ifile} '_FAST_detections.png'];
    print('-dpng', outfile);
end
