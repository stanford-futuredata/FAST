function [detections] = compare_hrsn_detections_with_catalog(ac, fpss, catTimes, catCCSum, baseDir, dataFolder)
...
for k=1:length(fpss.thresh)
    ...
    [detections.fpss(k), fpss, ind_match_fpss, ind_new_fpss, ind_missed_fpss] = ...
        find_matching_detections_above_thresh(k, time_window, num_fp, ncatalog, fpss, catTimes, catCCSum);
    ...
    if (flag_output_fpss)
        ...
        fprintf(fid, 'Mean CC sum of matching catalog detections = %7.6f\n', mean(fpss.catCCSum(ind_match_fpss)));
        fprintf(fid, 'Minimum CC sum of matching catalog detections = %7.6f\n', min(fpss.catCCSum(ind_match_fpss)));
        fprintf(fid, 'Maximum CC sum of matching catalog detections = %7.6f\n', max(fpss.catCCSum(ind_match_fpss)));
        C = [fpss.times(ind_match_fpss) fpss.sim(ind_match_fpss) fpss.catind(ind_match_fpss) fpss.cattime(ind_match_fpss) fpss.catCCSum(ind_match_fpss)];
        ...
        fprintf(fid, 'Mean CC sum of missed catalog detections = %7.6f\n', mean(catCCSum(ind_missed_fpss)));
        fprintf(fid, 'Minimum CC sum of missed catalog detections = %7.6f\n', min(catCCSum(ind_missed_fpss)));
        fprintf(fid, 'Maximum CC sum of missed catalog detections = %7.6f\n', max(catCCSum(ind_missed_fpss)));
        E = [catTimes(ind_missed_fpss) catCCSum(ind_missed_fpss)];
...
for k=1:length(ac.thresh)
    ...
    [detections.ac(k), ac, ind_match_ac, ind_new_ac, ind_missed_ac] = ...
        find_matching_detections_above_thresh(k, time_window, num_ac, ncatalog, ac, catTimes, catCCSum);
    ...
    if (flag_output_autocorr)
        ...
        fprintf(fid, 'Mean CC sum of matching catalog detections = %7.6f\n', mean(ac.catCCSum(ind_match_ac)));
        fprintf(fid, 'Minimum CC sum of matching catalog detections = %7.6f\n', min(ac.catCCSum(ind_match_ac)));
        fprintf(fid, 'Maximum CC sum of matching catalog detections = %7.6f\n', max(ac.catCCSum(ind_match_ac)));
        C = [ac.times(ind_match_ac) ac.sim(ind_match_ac) ac.catind(ind_match_ac) ac.cattime(ind_match_ac) ac.catCCSum(ind_match_ac)];
        ...
        fprintf(fid, 'Mean CC sum of missed catalog detections = %7.6f\n', mean(catCCSum(ind_missed_ac)));
        fprintf(fid, 'Minimum CC sum of missed catalog detections = %7.6f\n', min(catCCSum(ind_missed_ac)));
        fprintf(fid, 'Maximum CC sum of missed catalog detections = %7.6f\n', max(catCCSum(ind_missed_ac)));
        E = [catTimes(ind_missed_ac) catCCSum(ind_missed_ac)];
        fprintf(fid, '%7.5f %7.5f\n', E');
        fprintf(fid, '\n');
...
function [detinfo_k, newAlg, ind_match, ind_new, ind_missed] = ...
        find_matching_detections_above_thresh(k, time_window, num_newAlg, ncatalog, newAlg, catTimes, catCCSum)
    ...
    newAlg.catCCSum = zeros(num_newAlg,1); % CC sum over network for corresponding match in catalog
    ...
    for ifp=1:num_newAlg
        if (newAlg.sim(ifp) >= detinfo_k.thresh) % above new algorithm detection threshold
            ...
            if (diff_arr(ind_cat) <= time_window) % found a matching detection in catalog
                ...
                newAlg.catCCSum(ifp) = catCCSum(ind_cat);
