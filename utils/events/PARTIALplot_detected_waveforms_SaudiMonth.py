...
if len(sys.argv) != 3:
   print "Usage: python PARTIALplot_detected_waveforms_SaudiMonth.py <start_ind> <end_ind>"
   sys.exit(1)
...
[det_start_ind, det_end_ind, dL, nevents, nsta, tot_ndets, max_ndets, tot_vol, max_vol, peaksum, num_sta, diff_ind] = np.loadtxt(times_dir+'sort_nsta_peaksum_19sta_2stathresh_FinalUniqueNetworkDetectionTimes.txt', unpack=True)
out_dir = times_dir+'19sta_2stathresh_NetworkWaveformPlots/'
...
ts_dir = '/lfs/1/ceyoon/TimeSeries/SaudiMonth/data/'
for kk in range(IND_FIRST, IND_LAST):
   ...
   st = read(ts_dir+'waveforms*/Deci4.*2014.'+str(jday_start).zfill(3)+'*.SAC', format='SAC')
#   print len(st)
