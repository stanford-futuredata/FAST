...
if len(sys.argv) != 3:
   print "Usage: python PARTIALplot_detected_waveforms_OBSTest.py <start_ind> <end_ind>"
   sys.exit(1)
...
[det_start_ind, det_end_ind, dL, nevents, nsta, tot_ndets, max_ndets, tot_vol, max_vol, peaksum, num_sta, diff_ind] = np.loadtxt(times_dir+'sort_nsta_peaksum_35sta_2stathresh_FinalUniqueNetworkDetectionTimes.txt', unpack=True)
out_dir = times_dir+'35sta_2stathresh_NetworkWaveformPlots/'
...
ts_dir = '/lfs/1/ceyoon/TimeSeries/OBSTest/'
for kk in range(IND_FIRST, IND_LAST):
   ...
   day_str = str(start_time.year)+str(start_time.month).zfill(2)+str(start_time.day).zfill(2)
   print start_time, day_str

   st = read(ts_dir+'waveforms*/Deci1.bp*'+day_str+'*.sac', format='SAC')
#   print len(st)
#   print st.__str__(extended=True)
   ...
   st_slice.plot(equal_scale=False, show_y_UTC_label=False, size=(out_width,out_height), outfile=out_file)
